...
--------------------------------------------------------------------------------------------------------------------------------------
-- FUNCIÓN DE DELETE LÓGICO DE MOTIVOS DE RECHAZO
--------------------------------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION fun_delete_motivos_rechazo (wid_motivo_rechazo     tab_motivos_rechazo.id_motivo_rechazo%TYPE) RETURNS BOOLEAN AS
$BODY$
BEGIN

-- VALIDAR QUE EL ID DEL MOTIVO DE RECHAZO NO SEA NULO
    IF wid_motivo_rechazo IS NULL THEN
        RAISE EXCEPTION 'El ID motivo de rechazo no puede ser nulo.';
    END IF;

-- VALIDAR QUE EL MOTIVO DE RECHAZP EXISTA Y NO ESTÉ YA BORRADO
    IF NOT EXISTS (SELECT 1 FROM tab_motivos_rechazo WHERE id_motivo_rechazo = wid_motivo_rechazo AND ind_borrado = FALSE) THEN
        RAISE EXCEPTION 'El motivo de rechazo % no existe o ya se encuentra inactivo.', wid_motivo_rechazo;
    END IF;
